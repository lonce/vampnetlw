...
def train(
    ...
    print(f':::::::::::::::::starting training loop with num_iters={num_iters}.')
    ...
    with tracker.live:
        ...
        while not doneit :
            for tracker.step, batch in enumerate(train_dataloader, start=tracker.step):
                ...
                print(f':: tracker.step = {tracker.step}', file=sys.stderr)

                last_iter = (
                    tracker.step == num_iters - 1 if num_iters is not None else False
                )
                print(f':: in training loop with tracker.step = {tracker.step}, num_iters = {num_iters}, last_iter = {last_iter}.', file=sys.stderr)
                ...
                if last_iter:
                    print(f'last_iter =  {last_iter}. BREAKING train_loop')
                    doneit = True
                    break
